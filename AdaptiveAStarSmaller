...
##            print(grid.blocked)
    ...
    def display_grid(self):
        pygame.display.set_caption("Adaptive Repeated Forward A* Implementation")
        screen.fill(BLACK)
        done = False
        while done == False:
...
##        grid.next = exp
    ...
    def update_heuristics(self):
        i=1
##        print('Sgoal.g = %d' % (self.end.g))
        for grid in self.close:
            grid.h = self.end.g - grid.g
##            print('Update grid: %d,%d' % (grid.x, grid.y))

    def path_finder(self,true_maze):
        og_start = self.start
##        self.update_partial_maze(true_maze, Curr_start)
        counter = 1
